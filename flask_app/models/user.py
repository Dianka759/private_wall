...
class User:
    ...
    def validate(user):
        ...
        if not email_regex.match(user['email']):
            ...
            email_check = connectToMySQL("users_schema")
            query = "SELECT * FROM users WHERE email = %(email)s;"
            data = {
                "email": request.form["email"]
            }
            result = email_check.query_db(query, data)
            if result != ():  # if the result exists, it means the email exists already
                flash("Email already exists, please enter a new email", "register")
                is_valid = False
        if not pass_regex.match(user["password"]):
            flash(
                "Password must be minimum 8 characters, at least one letter and one number.", "register")
            is_valid = False
    ...
    def get_by_email(cls, data):
        query = "SELECT * FROM users WHERE email=%(email)s"
        user_db = connectToMySQL("users_schema").query_db(query, data)
        if len(user_db) < 1:
            return False
    ...
    @classmethod
    def get_recipient(cls, data):
        # shows messages from others to specific recipient
        query = "SELECT * FROM users JOIN messages ON users.id=messages.user_id JOIN users as recipient ON messages.friend_id=recipient.id WHERE recipient.id= %(id)s ORDER BY messages.created_at DESC;"
        return connectToMySQL("users_schema").query_db(query, data)
